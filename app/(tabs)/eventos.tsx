...
export default function EventosScreen() {
  ...
  // Nuevos estados para el DatePicker
  ...
  const handleCrearEvento = async () => {
    if (!titulo) {
      Alert.alert("Campos obligatorios", "Por favor ingresa un título para la juntada.");
      return;
    }
    try {
  ...
  return (
    <View style={styles.container}>
        ...
        <ScrollView>
          {eventos.map((evento: any) => (
            ...
            >
              ...
              <Text style={styles.eventoDetalle}>📅 {formatearFecha(evento.fecha_evento)}</Text>
              <Text style={styles.eventoDetalle}>📍 {evento.ubicacion || "Sin ubicación"}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      ) : (
              ...
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.inputGroup}>
                  ...
                  <View style={styles.datePickerContainer}>
                    <TouchableOpacity style={styles.datePickerButton} onPress={() => mostrarPicker('date')}>
                      <Text style={styles.datePickerText}>
                        📅 {fecha.toLocaleDateString('es-ES')}
                      </Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.datePickerButton} onPress={() => mostrarPicker('time')}>
                      <Text style={styles.datePickerText}>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <TextInput style={styles.input} value={ubicacion} onChangeText={setUbicacion} placeholder="Ej: Mi casa" />
                </View>
                    ...
                    <View style={styles.contactosContainer}>
                      {contactos?.map((contacto: any) => {
                        ...
                        return (
                          <TouchableOpacity
                            key={contacto.id}
                            style={[styles.contactoChip, isSelected && styles.contactoChipSelected]}
                            onPress={() => toggleContacto(contacto.id)}
                          >
                            <Text style={[styles.contactoText, isSelected && styles.contactoTextSelected]}>
                ...
                <View style={styles.modalActions}>
                  <TouchableOpacity style={styles.cancelButton} onPress={cerrarModalCrear}>
                    <Text style={styles.cancelButtonText}>Cancelar</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.submitButton, isSubmitting && styles.submitButtonDisabled]} onPress={handleCrearEvento} disabled={isSubmitting}>
                    {isSubmitting ? <ActivityIndicator color="#FFF" /> : <Text style={styles.submitButtonText}>Crear</Text>}
                  </TouchableOpacity>
                </View>
              </ScrollView>
            </View>
      ...
      <Modal animationType="fade" transparent={true} visible={modalDetalleVisible} onRequestClose={() => setModalDetalleVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, styles.detalleModal]}>
            {eventoSeleccionado && (
              <ScrollView showsVerticalScrollIndicator={false}>
                <Text style={styles.detalleTitulo}>{eventoSeleccionado.titulo}</Text>
                
                <View style={styles.detalleInfoGroup}>
                  <Text style={styles.detalleLabel}>📅 Cuándo:</Text>
                  <Text style={styles.detalleText}>{formatearFecha(eventoSeleccionado.fecha_evento)}</Text>
                </View>

                <View style={styles.detalleInfoGroup}>
                  <Text style={styles.detalleLabel}>📍 Dónde:</Text>
                  <Text style={styles.detalleText}>{eventoSeleccionado.ubicacion || "No especificado"}</Text>
                </View>

                <View style={styles.detalleInfoGroup}>
                  <Text style={styles.detalleLabel}>📝 Detalles:</Text>
                  <Text style={styles.detalleText}>{eventoSeleccionado.descripcion || "Sin descripción"}</Text>
                </View>

                {eventoSeleccionado.participantes_evento && eventoSeleccionado.participantes_evento.length > 0 ? (
                  <View style={styles.detalleInfoGroup}>
                    <Text style={styles.detalleLabel}>👥 Invitados ({eventoSeleccionado.participantes_evento.length}):</Text>
                    <View style={styles.contactosContainer}>
                      {eventoSeleccionado.participantes_evento.map((p: any) => (
                        <View key={p.contacto_id} style={styles.contactoChipSelected}>
                ...
                <TouchableOpacity style={styles.cerrarDetalleButton} onPress={() => setModalDetalleVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  submitButton: { flex: 1, padding: 15, borderRadius: 10, alignItems: "center", backgroundColor: "#28A745" },
  ...
  detalleLabel: { fontSize: 16, fontWeight: "bold", color: "#007AFF", marginBottom: 4 },
  detalleText: { fontSize: 16, color: "#444", lineHeight: 22 },
  eliminarButton: { marginTop: 15, padding: 15, borderRadius: 10, alignItems: "center", backgroundColor: "#D9534F" },
  eliminarButtonText: { color: "#FFF", fontWeight: "bold", fontSize: 16 },
  cerrarDetalleButton: { marginTop: 10, padding: 15, borderRadius: 10, alignItems: "center", backgroundColor: "#E0E0E0" },
